...
-- MAGIC %md
-- MAGIC transactions with gap date more than 3 days
...
with A as (
        select *,
        row_number() over(partition by customer_id order by transaction_date) rn,
        first_value(transaction_date) over(partition by customer_id) fv,
        transaction_date - row_number() over(partition by customer_id order by transaction_date) rnd
        from Transactions
       ),
       B as (

select *, date_diff(transaction_date,fv)-(rn-1) d, rnd-(date_diff(transaction_date,fv)-rn+1) ddd
from A)
select *, if(d>3, ddd, rnd) more_than_3
from B
